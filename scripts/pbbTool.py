...
# ------------------------------------------------
    ...
    parserExtract = subparsers.add_parser('extract', help='extract --pbbFile={pbbFile.pbb} --patternName=* --outputDir=.')
    ...
    parserExtract.add_argument("--outputDir", required=True, default=".", help="The directory to write the extracted patterns to")
    ...
    if args.command in ['backup']:
        ...
        args.func(args.pbbFile, args.patternName, args.outputDir)
    ...
    elif args.command in ['backup-epes']:
        if args.pbbFile is not None:
            for fileName in pathlib.Path().glob(args.pbbFile):
                args.func(PBB.fromFile(fileName))
        elif args.ipAddress is not None:
